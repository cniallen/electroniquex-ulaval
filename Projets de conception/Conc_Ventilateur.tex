...
\begin{itemize}
    \item Équipe de travail de 4 personnes formée sur le site de cours dans la section \texttt{Évaluations et résultats} en suivant le lien vers le \texttt{Projet de conception}.
    ...
    \item Une présentation orale de \SI{10}{\min} avec diapositives comportant les éléments indiqués dans la Table~\ref{tab:1} qui liste les critères d'évaluation et qui aura lieu pendant la plage horaire à réserver en inscrivant le nom de votre équipe dans le tableur en lien dans l'en-tête de cet énoncé. Cette présentation devra aussi démontrer la fonctionnalité du système réalisé au laboratoire ainsi que le respect des contraintes imposées à l'aide du plan de conception virtuelle. Des questions seront posées par le.les l'évaluateurs.trices pendant et/ou à la fin de la présentation.
