...
%Dernière modification CA: 5 février 2023
% 
%ToDo:
...
\item À partir des résultats de l'étape a) de la première partie,
    \begin{enumerate}
    \item présentez dans un tableau les mesures de résistance prises à deux fils et à quatre fils. Assurez-vous d'inclure les incertitudes et de présenter leur justification en légende.
    \item calculez le pourcentage d'écart entre chaque mesure du tableau et la valeur nominale de référence correspondante, puis à l'aide du vocabulaire international de métrologie (VIM), comparez l'exactitude des deux types de mesure de résistance. Pour quelle valeur de résistance le pourcentage d'écart est-il le plus important et quelle en est la cause?\\\textit{Ces pourcentages d'écart représentent des erreurs systématiques par rapport aux valeurs de référence, ce ne sont donc pas des incertitudes. Toutefois, l'incertitude sur ces erreurs peut être estimée!}
    \end{enumerate}
\item À partir des résultats des étapes c) et d) de la première partie, quelles étaient les mesures indirectes de résistance les plus significativement biaisées? À partir de ces résultats biaisés, déduisez la valeur de la résistance interne du voltmètre et celle de l'ampèremètre.
...
\item Identifiez les étapes (a,b,c, etc.) dans chaque partie de la section "Manipulations" du protocole qui peuvent être faites en préparation avant le laboratoire.
