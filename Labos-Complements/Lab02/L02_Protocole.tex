...
%Voir mon cahier d'enseignement pour comment aborder les moyennes d'ensemble disperses avec des éléments qui varient (vs une même meusre répétée) et travailler ça à partir de résistances avec les tolérances appropriées...
    ...
    %Tester que la réponse de la prochaine sous-question n'est pas influencée par la trop poche tolérance des résistances ! Pis faudrait ben s'affranchir assez des tolérances pour démontrerle point principal que de petites résistances doivent être mesurées à 4 fils!
...
\item Dans votre analyse des résultats des étapes c) et d) de la première partie, 
    \begin{gradescope}
    %Le pronom dans les phrases suivantes doit référer aux résistances, mais je ne suis pas certaine que c'Est réussi...
    ...
    %INTRODUIRE TOUS LES SYMBOLES QUI NE L'ONT PAS ÉTÉ JUSQU'À MAINTENANT puis Inspiration pour d'autres mauvais choix de réponse pour ajout par Annie-Claude à partir de celles en H23!
    \end{itemize}
...
\item Est-ce que l'incertitude d'une mesure dépend \textit{uniquement} de l'exactitude de l'instrument utilisé? Supportez votre réponse à l'aide d'un exemple tiré de chacun des deux premiers laboratoires. % <- Pour Annie-Claude s'il y a vraiment moyen de tourner ça en choix de réponse intéressant!
\item Identifiez les étapes dans chaque partie de la section "Manipulations" du protocole qui peuvent être faites en préparation avant le laboratoire :\\ $\blacktriangleright$ 1a) $\blacktriangleright$ 1b) $\blacktriangleright$ 1c) $\blacktriangleright$ 1d) $\blacktriangleright$ 2a) $\blacktriangleright$ 2b) $\blacktriangleright$ 2c) $\blacktriangleright$ 2d) $\blacktriangleright$ 2e) $\blacktriangleright$ 2f) $\blacktriangleright$ 2g) $\blacktriangleright$ 2h) %sauter toute étape avec le moindre potentiel d'ambiguïté, genre vérification par l'équipe d'enseignement? ou modifier l'étape concernée?
