...
%Dernière modification CA: 5 septembre 2022
%
%**ToDo**: 
...
\end{itemize}
\vspace{1ex}
\noindent{\itshape La recherche va donc bien au-delà de faire les expériences proprement dites et le \texttt{Cahier de recherche} est votre journal de bord pour vous accompagner à toutes ces étapes. N'hésitez pas à y écrire vos pensées en tout temps dans le cahier OneNote de votre équipe <\url{https://www.onenote.com/notebooks}>, surtout en ce premier cours de laboratoire où son évaluation se concentre sur la consignation de la recherche en temps réel. Le guide de rédaction du cahier spécifiant des consignes de traçabilité et de continuité de la recherche sera noté seulement de manière formative cette session en vue de progresser vers une évaluation complète dans les cours expérimentaux subséquents.
\vspace{3ex}

La compréhension du contenu en laboratoire sur l'électronique, la métrologie, l'analyse des résultats et la statistique, quant à elle, repose sur une évaluation des lectures préparatoires (quiz sur le site de cours) et des questions de la section \nameref{sec:grade} <\url{https://www.gradescope.com/}>. La toute dernière de ces questions évaluera aussi votre progression en rédaction scientifique et technique sur les 4 premiers laboratoires qui, en rassemblant les parties, forment l'essentiel de votre premier rapport de laboratoire universitaire.}
...
\section[Lectures préparatoires]{Lectures préparatoires\footnote{Ces lectures font l'objet d'un bref quiz sommatif, accédez-le en arrivant à chaque laboratoire sur le site de cours.}} \label{sec:prep}
\vspace{-2ex}
...
\item le protocole de ce laboratoire en portant une attention particulière à la section \textbf{\nameref{sec:grade}}: à la sortie de chaque séance, il faut s'assurer d'avoir toutes les données correctes et autres informations nécessaires pour répondre à ces questions.
\end{itemize}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%% \noindent\framebox{\parbox{\dimexpr\linewidth-2\fboxsep-2\fboxrule}{AJOUTER UN BEAU COMMENTAIRE COMME DE QUOI LES LECTURES EST JUSTE LE DÉBUT DE LA PREP, RÉFLÉCHIR AUX CALCULS POSSIBLES ET TOUTE POUR GAGNER DU TEMPS, EN PARTICULIER LORSQUE PROGRAMMATION}}%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\item Vous aurez aussi remarqué que la combinaison acier--aluminium donnait une tension qui fluctuait autour de zéro. Pourquoi en est-il ainsi?\\a) L'acier est oxydé.\\b) Acier et aluminium commencent les deux par la lettre «a», donc ils ne peuvent réagir ensemble.\\c) Les deux métaux ont la même électronégativité.
