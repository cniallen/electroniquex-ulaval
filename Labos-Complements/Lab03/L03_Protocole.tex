...
(0,0) to[R=1.2~k$\Omega$] 
(2,0) to[ammeter] 
(4,0) to[short] (4,2)
(0,0) to(0,2) 
(4,2) to[V, l_=$0~V-6~V$] (0,2)
;\end{circuitikz}
...
(2,0) to(4,0) 
(0,2) to[V, l_=$0~V-25~V$] (0,0)
(0,0) to(2,0) 
(4,3) to[V=$0~V-6~V$] (4,0)
(2,3) to[ammeter] (4,3)
;\end{circuitikz}
...
\centering
\begin{circuitikz} \draw
(0,2.5) to[V, l_=$v_{\textrm{s}}$] (0,0)
(0,2.5) to[R=1~k$\Omega$] 
(3,2.5) to[european resistor,l^=regulateur] (5,2.5) 
node[right]{$v_{\mathrm{out}}$}
(0,0) node[ground]{} to[short] 
(4,0) to[short] (4,2.26)
;\end{circuitikz}
...
\centering
\begin{circuitikz} \draw
(0, 2.5) to[V, l_=$v_{\mathrm{s}}$] (0,0)
(0,0) node[ground]{}
(0,2.5) to[R=1~k$\Omega$] (3,2.5)
(0,0) to[short] (3,0) to[zD] (3,2.5)
(3,2.5) to[short] 
(4,2.5) node[right]{$v_{\mathrm{out}}$}
;\end{circuitikz}
...
\end{gradescope}
\vspace{-0.5ex}
%\textit{\footnotesize Indice: Ces deux composants remplissent une fonction dans un circuit à courant alternatif, car leur relation entre tension et courant dépend d'un taux de VARIATION par rapport au temps. Ils ne servent donc à rien en courant continu et il s'agit de comprendre physiquement pourquoi.} Je le donne maintenant en explication de correction sur Gradescope.
...
\begin{gradescope}[resume]
    ...
    \end{itemize}
    %\item Comparez les régulateurs de tension de la Partie 5. Tracez pour chacun la courbe de la tension de sortie $v_{\mathrm{out}}$ en fonction de la tension d'entrée $v_{\mathrm{s}}$. Afin de faire une meilleure comparaison, normalisez vos valeurs (divisez les tensions de sortie de chaque composant par la valeur maximale obtenue) et affichez les deux courbes sur le même graphique. SERAIT PLUS INTÉRESSANT DE TRAVAILLER LA MINIMISATION DE RÉSIDUS POUR PLACER (FIT) LE MODÈLE DE LA MEILLEURE DROITE VERTICALE D'UNE FONCTION IDÉALE DE RÉGULATION EN TENSION. Connecte avec leur 1er vrai fit du cours dans l'analyse de la diode qui suit, mais ces questions d'analyses sont les plus longues et je ne pense pas qu'il reste assez de temps pour commencer pendant la séance de laboratoire, à vérifier.
