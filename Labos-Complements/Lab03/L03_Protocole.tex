...
Après avoir caractérisé le comportement de composants à partir de leurs courbes $i$--$v$, on verra dans cette dernière partie comment il est possible de prendre avantage de ces comportements pour concevoir des fonctions utiles dans un circuit. L'exemple particulier de la régulation de tension avec une diode zener sera donné.

Les régulateurs de tension sont utilisés pour fixer la tension aux bornes d'une charge, indifféremment du reste du circuit. Il existe plusieurs façons de concevoir un régulateur; vous avez dans votre coffre un modèle de régulateur NCV7805, qui est un exemple d'un circuit intégré accomplissant cette fonction.

a) Pour comprendre la fonction du régulateur de tension, commencez par monter le circuit de la figure \ref{sch-reg-1} sur la plaquette de montage. Consultez la fiche technique du régulateur de tension afin de faire les bons branchements.
...
b) Faites varier la source de 0~volt à 14~volts par incréments de 2~volts. À l'aide d'un multimètre en mode voltmètre (DCV), mesurez à chaque fois la tension $v_{\mathrm{out}}$. Notez qu'il n'est plus nécessaire d'utiliser \textit{LabVIEW} à ce stade-ci.

Une version très simpliste de ce circuit intégré régulateur de tension sera maintenant conçue à partir d'une caractéristique de la courbe $i$--$v$ d'une diode Zener.
