...
%***ToDo***
% - préparer un signal de bruit pour montrer comment on se rapproche de sinus/cosinus avec Q qui augmente dans un filtre passe-bande + tout le stock ajouté par Jé ci-dessous.
...
(opamp.+) to[short] (-1.2,-0.5) to [C,l=1~$\mu$F] (-1.2,-2.5) node[ground]{}
(opamp.-) to[short] (-1.2,0.5) to[short] (-1.2,1.5) to[short] (1.2,1.5) to[short](1.2,0)
(opamp.out) to[short,-o] (1.5,0) node[right]{$V_{\mathrm{out}}$}
(-4,-0.5) node[left]{$V_{\mathrm{g}}$} to[R=270~$\Omega$,o-] (-1.2,-0.5) 
;\end{circuitikz}}
\subcaptionbox{}
{\begin{circuitikz} \draw
(0,0) node[op amp](opamp){}
(opamp.+) to[short] (-1.2,-0.5) to[short] (-1.2,-1) node[ground]{}
(opamp.-) to[short] (-1.2,0.5) to[short] (-1.2,1.5) to[R=270~$\Omega$] (1.2,1.5) to[short] (1.2,0)
(opamp.out) to[short,-o] (1.5,0) node[right]{$V_{\mathrm{out}}$}
(-1.2,1.5) to[short] (-1.2,2.8) to[C=1~$\mu$F] (1.2,2.8) to[short] (1.2,1.5)
(-2,-0.5) node[left]{$V_{\mathrm{g}}$} to[short,o-](-1.2,-0.5)
;\end{circuitikz}}
\subcaptionbox{}
{\begin{circuitikz} \draw
...
;\end{circuitikz}}
\caption{\label{sch-RC-ordre3}\textbf{Sous-objectif} : examiner comment les filtres (a) RC série passif de premier ordre, (b) RC série actif non-inverseur, (c) RC parallèle actif inverseur, (d) RC passif de troisième ordre, (e) Butterworth passif en topologie Cauer de troisième ordre et (g) Butterworth actif en topologie Sallen-Key de troisième ordre, vont dévier du comportement idéal d'un filtre passe-bas. Ce spectre idéal du gain est une fonction échelon de Heaviside en symétrie miroir centrée sur la fréquence de coupure. Cette symétrie de parité paire est seulement pour exprimer l'opération $H(-x)\rightarrow H(x)$.}
\end{figure}

\noindent\textit{Pour vous aider à tracer rapidement les diagrammes de Bode, vous pouvez utiliser le VI LabVIEW disponible sur le site du cours dans la section dédiée à cet atelier.}
