...
%Inverser intro aux dB (aussi échelle logarithmique en général, quoique ce serait bien avant le lab III pour cette analyse) et Bande passante-fréquence de coupure pour clarifier connexion -3 dB et $2^{-1/2}$ linéaire en amplitude. Peut-être même la notion de filtres avant ça. P.S. Ça $\frac{1}{\sqrt{2}}=2^{-1/2}$ c'est pas beau dans le texte, garder dans les diapos.
%Rajouter illustration des cas idéaux de types de filtres (transitions verticales) dans le coin des diagrammes de Bode puis introduire déviation en pente dB/decade, ça va mieux connecter avec le dernier sous-objectif de l'atelier.
%
